...
static const int MAX_NUM_CLIENTS = 77;
struct tcp_pcb *arrayPCB[MAX_NUM_CLIENTS] = {0};
...
void (*SocketFuncConnect)(int _id) = 0;                          // this function will be called every time a new connection
void (*SocketFuncReciever)(int _id, const char *_buffer, uint _length) = 0;    // this function will be called when a message is recieved from any client

void CloseConnection(struct tcp_pcb *_tpcb, struct State *_ss) {
    tcp_arg(_tpcb, NULL);
    tcp_sent(_tpcb, NULL);
    tcp_recv(_tpcb, NULL);
    tcp_err(_tpcb, NULL);
    tcp_poll(_tpcb, NULL, 0);
...
void Send(struct tcp_pcb *_tpcb, struct State *_ss) {
    ...
    int IndexPCBinArray(struct tcp_pcb *_tpcb) {
        for(int i = 0; i < MAX_NUM_CLIENTS; i++) {
            if(arrayPCB[i] == _tpcb) {
                return i;
            }
    ...
    err_t CallbackOnRecieve(void *_arg, struct tcp_pcb *_tpcb, struct pbuf *_p, err_t _err) {
        ...
        if(_p == NULL) {
            ...
            if(ss->p == NULL) {
                ...
                //tcp_sent(_tpcb, CallbackOnSent);
                //Send(_tpcb, ss);
                SocketFuncReciever(IndexPCBinArray(_tpcb), (char*)_p->payload, _p->len);
    ...
    bool TCP_SOCKET_Init(void (*_funcConnect)(int _id), void (*_funcReciever)(int _id, const char *_buffer, uint _length)) {
    ...
    bool SendToAll(const char *_buffer, uint _length) {
        for(int i = 0; i < MAX_NUM_CLIENTS; i++) {
            TCP_SOCKET_SendToOne(i, _buffer, _length);
        }
        return true;
    }
    ...
    bool TCP_SOCKET_SendToOne(int _id, const char *_buffer, int _length) {
        if(arrayPCB[_id]) {
            struct pbuf *tcpBuffer = pbuf_alloc(PBUF_RAW, _length, PBUF_POOL);
            tcpBuffer->flags = 1;
            pbuf_take(tcpBuffer, _buffer, _length);
            struct State *ss = (struct State*)mem_malloc(sizeof(struct State));
            ss->p = tcpBuffer;
            Send(arrayPCB[_id], ss);
            mem_free(ss);
        }
        return (arrayPCB[_id] != 0);
    }

    int TCP_SOCKET_AllConnections() {
        int retValue = 0;
        for(int i = 0; i < MAX_NUM_CLIENTS; i++) {
            if(arrayPCB[i]) {
                retValue++;
