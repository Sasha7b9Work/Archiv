     ...
     int stackptr)
{
    ...
    if( plan.plan(entryoffset+3)==fftrealcooleytukeyplan )
    {
        ...
        for(i = 0; i <= n1/2-1; i++)
        {
            ...
            ap::vmove(&a(offs), &plan.tmpbuf(0), ap::vlen(offs,offs+2*n2*2-1));
        }
        if( n1%2!=0 )
        {
     ...
     ap::real_1d_array& buf)
{
    ...
    ap::vmove(&a(astart), &buf(0), ap::vlen(astart,astart+2*m*n-1));
}
     ...
     ap::real_1d_array& buf)
{
    ...
    ap::vmove(&a(astart), &buf(0), ap::vlen(astart,astart+m*n-1));
}
