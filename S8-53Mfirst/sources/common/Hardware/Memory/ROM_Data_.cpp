// 2021/04/27 11:55:30 (c) Aleksandr Shevchenko e-mail : Sasha7b9@tut.by
#include "defines.h"
...
// —жать хранимые данные - перераспределить их по секторам таким образом, чтобы осталось полностью свободными как
// мимиум два сектора
...
const DataSettings *ROM::Data::Read(uint numInROM)
{
    for (int i = 0; i < NUM_SECTORS; i++)
    {
        const ReadingFrame *dr = sectors[i]->ReadData(numInROM);
