...
const ReadingFrame *PacketROM::UnPack() const
{
    if (!IsValid() || !IsData())
    {
        return nullptr;
    ...
    return const_cast<ReadingFrame *>
        (reinterpret_cast<const ReadingFrame *>(reinterpret_cast<const uint8 *>(this) + sizeof(PacketROM)));
}
...
const PacketROM *Sector::WriteData(uint numInROM, const DataSettings *ds) const
{
    ...
    // ≈сли пакет при записи вылезет за границу сектора. ƒаЄм запас - в конце сектора должны остатьс€ миниму 4 байта,
    ...
    const_cast<DataSettings *>(ds)->num_in_rom = numInROM;
...
const ReadingFrame *Sector::ReadData(uint numInROM) const
{
...
void Sector::GetDataInfo(bool existData[ROM::Data::MAX_NUM_SAVED_WAVES]) const
{
    ...
    while (packet && !packet->IsFree())
    {
        if (packet->IsData())
        {
            const ReadingFrame *ds = packet->UnPack();
            if (ds)
            {
                existData[ds->Settings().num_in_rom] = true;
            }
        }
        packet = packet->Next();
    }
...
{
    const ReadingFrame *dr = packet->UnPack();
    ...
    return (int)(dr->Settings().num_in_rom);
}
